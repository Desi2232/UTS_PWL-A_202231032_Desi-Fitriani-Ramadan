            ...
            @foreach ($books as $book)
                <a href="{{ route('books.show', $book->slug) }}"
                    ...
                    @if ($dipinjam == false)
                        ...
                        @endif
                    @endif
                    <img src="{{ asset('storage/' . $book->image) }}" alt="{{ $book->title }}" class="w-full h-96 object-cover rounded">                    <h1 class="mt-2 font-bold text-lg text-gray-700 truncate group-hover:truncate-none peer">
                        {{ $book->title }}</h1>
                    <div
                    ...
                    <div class="text-sm flex text-gray-700 items-center font-medium">
                        ...
                        <span class="ml-2">{{ $book->thn_terbit }}</span>
                    </div>
                    <div class="text-sm flex text-gray-700 items-center font-medium">
                        <i data-feather="layers" width="16px"></i>
                        <span class="ml-2">{{ $book->halaman }} Pages</span>
                    </div>
                </a>
            @endforeach
        </div>
    ...
    <div class="p-4">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('books.create') }}"
                class="transition-all duration-500 bg-[#2A4759] rounded-lg text-white font-medium px-5 py-2.5 focus:ring-2
                focus:ring-blue-500 focus:ring-offset-2 text-center hover:bg-[#27548A] text-sm">Tambah
                Buku</a>
        </div>
        <div class="mt-6">
